...
The `abrupt-subagent-liveness-v1` revision raises the JavaScript gzip ceiling from 105,500 to 105,800 bytes for exact native PID/start-time reconciliation of `working` and `attention` session rows. The v0.1.8 candidate measures 105,685 bytes gzip, 284 bytes above the accepted v0.1.7 complete-catalog build (105,401 bytes) and 115 bytes below the new ceiling. The added bytes keep per-conversation liveness targets separate from process-grouped Runtime history, preserve the 64-target safety cap, and stop background sampling when no eligible active identity remains; no dependency or visual-asset payload changed.
...
The `agent-halo-material-tabs-v1` revision raises the primary CSS gzip ceiling from 13,650 to 17,000 bytes and JavaScript gzip from 105,800 to 106,500 bytes. The measured candidate is 16,708 bytes CSS gzip and 106,176 bytes JavaScript gzip. The bounded allowance covers view-owned panel widths, the original Sessions / Focus / Usage / Runtime / Services information architecture with separate Setup, Agent Halo-owned discrete tab markers, one coherent tokenized color personality per surface, board-owned vertical scrolling with complete material edges, responsive container-query navigation, and the reproducible 9,682-byte local repeating noise tile. It does not retain the rejected Workspace / Insights dashboard grouping, its navigation anatomy, or the superseded orange-on-dark open-panel recipe. The collapsed hardware-notch behavior, core `dist/`, mascot assets, MediaPipe runtime, local-camera privacy, and legacy-asset constraints remain unchanged.
