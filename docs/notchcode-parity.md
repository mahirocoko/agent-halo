...
Current desktop focus is intentionally narrower: `focus_terminal` is a native macOS fallback for supported terminals. Ghostty matching uses its scripting dictionary. Warp exact matching first tries optional Warp local control via `warpctrl tab activate --tab-title ...` when the user has enabled Settings > Scripting and installed `warpctrl`; otherwise it uses accessibility window-title matching plus Tab-menu cycling because Warp does not expose the same terminal/tab scripting model. The local mod sets a terminal title from workspace + conversation id so Warp tabs have a unique hint without requiring one agent per project. When no exact match is found, Agent Halo activates a supported terminal and reports app-level activation. Exact pane focus should use future metadata such as terminal title mapping or `TMUX_PANE` plus `tmux select-pane`.
