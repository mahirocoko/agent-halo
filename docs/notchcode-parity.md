...
| Pointer/keyboard-expand dropped sheet | Hover, click, Enter, or Space opens the view-owned desktop companion (`.halo-surface`); declared per-view widths (Sessions 560, Focus 540, Usage 700, Runtime 660, Services 620, Setup 640, session detail 520) clamp to the available screen. Tauri `set_panel_open` resizes the native transparent window. Escape closes overview or returns detail/Setup to Sessions with focus restoration. | Done |
