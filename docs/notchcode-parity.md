...
| Local Pomodoro | Header Pomodoro tab provides customizable duration/cadence settings with 25/5/15/every-4 defaults, active-duration snapshots, persisted absolute deadlines, pause/reset/skip, collapsed countdown, and silent native completion alerts. Attention/Error overrides the timer in the notch. | Done |
| Movement Break | Opt-in Completion Pet action starts one local camera stream plus a bundled shoulder-line challenge only after explicit consent; white-to-green descent progress stays local and the validated main Pomodoro owner starts the prepared break at completion. | Phase 1 |
| Pomodoro regressions | `apps/desktop/tests/demo-pomodoro.spec.ts` covers cycle transitions, controls, reload persistence, collapsed completion/countdown, agent precedence, and native schedule/cancel calls. | Covered |
