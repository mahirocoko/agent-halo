...
The native desktop also reconciles `working` and `attention` rows against the existing exact runtime identity (`sourcePid` plus process start time). While either state is present it samples the 64 most recent live-status targets every five seconds, prioritizing them over Runtime-history targets when both jobs share one native request. A missing process or reused PID immediately tombstones that exact runtime identity and projects the row as `inactive`, without fabricating a completion event or waiting for the longer event-only fallback. Browser/demo and non-native session projections retain the event-aware timeout because they cannot inspect local process identity safely; Runtime demo scenarios may still simulate ended process targets for that panel's own tests.
