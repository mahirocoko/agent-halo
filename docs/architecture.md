...
The Phase 1 Completion Pet adds one hidden transparent `pet` WebView routed before the main `App` mounts. The main renderer remains the sole Pomodoro owner; Pet is an isolated action surface backed by validated native commands and its own display-safe persisted position. The opt-in Movement Break reuses that surface after an explicit click: one ephemeral `getUserMedia` stream feeds both the mirrored preview and bundled local MediaPipe shoulder tracker, while a summon-bound native attempt token returns completion through the validated Pet action queue. See `docs/pet.md` and `docs/movement-break.md`.
