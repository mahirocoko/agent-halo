...
- **Top-Level Tabs**:
  ...
  - **Usage**: Codex, Antigravity, Claude Code, and Cursor quota/token cards shown together, with resizable widths and truthful per-provider unavailable/offline diagnostics.
...
| **Usage** | Four local provider cards on one board; each card keeps its own metrics, loading/error state, and scroll position, with resizable widths |
...
All four provider cards appear together rather than behind a provider switcher. Adjacent dividers resize widths by pointer or arrow keys; widths are saved locally and Reset layout restores equal columns. At narrow panel widths the cards stack in provider order and share vertical scrolling. The Usage settings button switches to a full-page settings state with refresh, quota-display, reset-time, clock-format, and provider visibility controls; Back to Usage returns to the provider board.
...
- Cursor quota in Agent Halo comes from the direct Cursor provider read (`GetCurrentPeriodUsage`), not from Cursor hooks or the Cursor statusline. Those surfaces carry lifecycle and session context, not account usage. After a successful period-usage response, Agent Halo is the trusted producer for Mahiro Herdr Sidebar: it best-effort publishes only a sanitized `{ fetched, failed: false, windows }` snapshot to `cursor.json` (default `~/.letta/mods/mahiro-usage/`, or a non-empty absolute `MAHIRO_HERDR_USAGE_CACHE_DIR`). Valid Auto and API windows stay in that order; plan name, credits, tokens, history, credentials, and the raw provider payload stay out. Sidebar only reads that cache. A publication failure does not change the Cursor Usage card.
- Cache publication follows Agent Halo's existing renderer-owned Usage refresh while the desktop app is running. It does not add a provider poller; when Agent Halo is not running, Mahiro Herdr Sidebar clears the Cursor rows after its bounded cache freshness window.
