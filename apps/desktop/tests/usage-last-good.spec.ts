...
test('Usage keeps provider cards on one board and persists accessible resizing', async ({ page }) => {
  ...
  const persisted = await page
    .locator('.usage-provider-surface')
    .nth(0)
  ...
  expect(
    await page.evaluate(() => JSON.parse(window.localStorage.getItem('agent-halo.usage-layout.v1') ?? 'null')),
...
test('Usage settings can hide and restore provider cards', async ({ page }) => {
  ...
  expect(
    await page.evaluate(() => JSON.parse(window.localStorage.getItem('agent-halo.usage-visibility.v1') ?? 'null')),
  ).toEqual(['codex', 'agy', 'claude', 'cursor'])
})
...
test('Usage keeps Codex values visible and labels them outdated after a refresh failure', async ({ page }) => {
  ...
  const refreshGeometry = await page.getByRole('button', { name: 'Refresh usage' }).evaluate((button) => {
    ...
    return {
      ...
      shape: button.dataset.surfaceControlShape,
    }
  })
  expect(refreshGeometry).toEqual({ width: 28, height: 28, radius: '50%', shape: 'circle' })
  const monochromePaint = await page.evaluate(() => ({
    providerIcon: getComputedStyle(document.querySelector<HTMLElement>('.usage-provider-title .usage-provider-icon')!)
      .backgroundColor,
    onlineDot: getComputedStyle(document.querySelector<HTMLElement>('.usage-card-status-dot')!).backgroundColor,
    trendBars: [...document.querySelectorAll<HTMLElement>('.usage-trend-bar')].map(
...
test('Usage meters communicate remaining quota with monochrome structure and copy', async ({ page }) => {
  ...
  expect(statusPaint[1]).toMatchObject({
  ...
  const dotPaint = await meters
    .locator('.usage-status-dot')
