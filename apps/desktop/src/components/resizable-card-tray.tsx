...
const normalizeDefaults = (defaults: number[]): number[] => {
  const sum = defaults.reduce((total, ratio) => total + ratio, 0)
  return sum > 0 ? defaults.map((ratio) => ratio / sum) : defaults
}
...
const normalizeRatios = (ratios: number[], defaults: number[]): number[] => {
  ...
  if (!Number.isFinite(sum) || sum <= 0 || Math.abs(sum - 1) > 0.1) return normalizedDefaults
...
export const useResizableCardLayout = ({
  ...
  const defaults = useMemo(
    () => normalizeDefaults(defaultRatios ?? specs.map((spec) => spec.defaultRatio)),
    [defaultRatios, specs],
  )
  ...
  const handlePointerMove = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const nextLeft = Math.max(
      minWidth,
      Math.min(combined - minWidth, drag.startWidths[left] + event.clientX - drag.startX),
    )
  ...
  const handleKeyDown = (divider: number, event: KeyboardEvent<HTMLDivElement>) => {
    ...
    const nextLeft = Math.max(
      minWidth,
      Math.min(combined - minWidth, widths[divider] + (event.key === 'ArrowRight' ? 16 : -16)),
    )
...
export const ResizableCardDivider = ({
  ...
  return (
    <div {...props} className={[props.className, className].filter(Boolean).join(' ')}>
      <div className="card-resize-grip" aria-hidden="true" />
    </div>
  )
}
