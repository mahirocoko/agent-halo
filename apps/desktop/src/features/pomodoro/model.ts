...
export const getPomodoroRemainingMs = (state: IPomodoroState, now: number): number =>
  state.status === 'running' && state.endsAt !== null ? Math.max(0, state.endsAt - now) : Math.max(0, state.remainingMs)
...
export const resetAllPomodoro = (settings = DEFAULT_POMODORO_SETTINGS): IPomodoroState => createPomodoroState(settings)
...
export const reconcilePomodoro = (
  state: IPomodoroState,
  now: number,
  settings = DEFAULT_POMODORO_SETTINGS,
): IPomodoroState =>
...
export const getPomodoroNotificationCopy = (
  state: IPomodoroState,
  settings = DEFAULT_POMODORO_SETTINGS,
