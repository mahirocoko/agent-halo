...
const createRunId = (): string => {
  try {
    ...
    return `pomodoro-${Date.now()}-${Math.random().toString(16).slice(2)}`
  }
}
...
export const usePomodoro = (canUseNativeNotifications: boolean, preferCompletionPet = false): IUsePomodoroResult => {
  ...
  const [notificationPermission, setNotificationPermission] = useState<PomodoroNotificationPermission>(
    canUseNativeNotifications ? 'notDetermined' : 'unsupported',
  )
  ...
  const pause = (): void => {
    ...
    commitState(
      reconciled !== current ? reconciled : current.status === 'running' ? pausePomodoro(current, actionAt) : current,
    )
  }
  ...
  const updateSettings = (nextSettings: IPomodoroSettings): void => {
    ...
    commitState(current)
    if (!canUseNativeNotifications || current.status !== 'running' || current.endsAt === null || current.runId === null)
      return
    ...
    enqueueNativeOperation(async () => {
      try {
        ...
        if (nativeRequestVersionRef.current !== requestVersion) return
        const latest = stateRef.current
        if (latest.runId === current.runId && latest.status === 'running')
          commitState({ ...latest, notificationScheduled: true })
      } catch (error) {
        if (nativeRequestVersionRef.current === requestVersion)
          setNotificationError(error instanceof Error ? error.message : 'Could not update macOS notification')
      }
    })
  }
  ...
  const resolveCompletionWithPet = async (handoffDeadlineMs: number): Promise<boolean> => {
    ...
    const operation = nativeOperationRef.current
      ...
      .then(async () => {
        ...
        try {
          ...
          if (nativeRequestVersionRef.current === requestVersion)
            setNotificationError(
              error instanceof Error ? error.message : 'Could not replace notification with Completion Pet',
            )
          return false
        }
        if (nativeRequestVersionRef.current !== requestVersion) return false
        ...
        if (
          ...
          latestCompletion.id === completionId &&
          latestCompletion.notificationScheduled
        ) {
          commitState({
            ...latest,
    ...
    nativeOperationRef.current = operation.then(
      () => undefined,
      () => undefined,
    )
    return operation
    ...
    const operation = nativeOperationRef.current
      ...
      .then(async () => {
        ...
        try {
          ...
          if (nativeRequestVersionRef.current === requestVersion)
            setNotificationError(
              error instanceof Error ? error.message : 'Could not restore Completion Pet notification fallback',
            )
          return false
        }
        if (nativeRequestVersionRef.current !== requestVersion) return false
  ...
  const view = useMemo<IPomodoroView>(
    () => ({
      ...
      cyclePosition:
        state.phase === 'long-break' ? settings.longBreakEvery : state.completedFocusSessions % settings.longBreakEvery,
      completionVisible:
        state.lastCompletion !== null &&
        now >= state.lastCompletion.observedAt &&
        now - state.lastCompletion.observedAt <= POMODORO_COMPLETION_SIGNAL_MS,
  ...
  return {
    ...
    skip,
    updateSettings,
    resolveCompletionWithPet,
    restoreCompletionNotificationFallback,
  }
}
