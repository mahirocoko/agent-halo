...
const readCompletion = (value: unknown, now: number): IPomodoroCompletion | null => {
  ...
  if (
    !isSafeTimestamp(candidate.completedAt) ||
    !isSafeTimestamp(candidate.observedAt) ||
    candidate.observedAt > now + MAX_CLOCK_SKEW_MS ||
    !PHASES.includes(candidate.completedPhase as PomodoroPhase) ||
    !PHASES.includes(candidate.nextPhase as PomodoroPhase)
  )
    return null
  return {
    id: candidate.id,
...
export const normalizePomodoroState = (
  ...
  const phaseDurationMs =
    schemaVersion === 2 && typeof candidate.phaseDurationMs === 'number' && Number.isFinite(candidate.phaseDurationMs)
  ...
  const state: IPomodoroState = {
    ...
    completedFocusSessions:
      typeof candidate.completedFocusSessions === 'number' && Number.isSafeInteger(candidate.completedFocusSessions)
        ? Math.min(MAX_COMPLETED_FOCUS_SESSIONS, Math.max(0, candidate.completedFocusSessions))
        : 0,
    phaseDurationMs,
    remainingMs:
      typeof candidate.remainingMs === 'number' && Number.isFinite(candidate.remainingMs)
        ? Math.min(phaseDurationMs, Math.max(0, candidate.remainingMs))
        : phaseDurationMs,
  ...
  if (state.status === 'running' && (state.endsAt === null || state.runId === null))
    return resetInvalidRunningState(state, settings)
  if (state.status === 'running' && state.endsAt !== null && state.endsAt > now + phaseDurationMs + MAX_CLOCK_SKEW_MS)
    return resetInvalidRunningState(state, settings)
