...
import type { IUseStopwatchResult } from './use-stopwatch'
      ...
      <div className="stopwatch-primary-zone">
        <div className="stopwatch-heading-row">
          <span className="stopwatch-icon" aria-hidden="true">
            <Clock3 size={13} strokeWidth={2.3} />
          </span>
          <h2 id="stopwatch-heading">Stopwatch</h2>
          <span className="stopwatch-status" data-status={semanticStatus}>
        ...
        <div className="stopwatch-controls">
          {running ? (
            <SurfaceControl
              className="stopwatch-control primary"
          ...
          <SurfaceControl
            ...
            data-tauri-drag-region="false"
            aria-label={
              discardArmed ? 'Confirm discard current Stopwatch session' : 'Discard current Stopwatch session'
            }
          >
      ...
      <div className="stopwatch-support-zone">
        <div className="stopwatch-history-heading">
          ...
          >
            ...
            {clearHistoryArmed ? 'Confirm clear' : 'Clear history'}
          </SurfaceControl>
        </div>
        ...
        {historyGroups.length === 0 ? (
          ...
          <div className="stopwatch-history-list">
            {historyGroups.map((group) => (
              <section
                className="stopwatch-history-group"
                key={group.key}
                aria-label={`${group.label} Stopwatch history`}
              >
                <div className="stopwatch-history-date">
                  <span>{group.label}</span>
                  <span>{formatStopwatchElapsed(group.totalMs)}</span>
                </div>
                <ol>
