...
export const normalizeStopwatchState = (value: unknown, now = Date.now()): IStopwatchState => {
  ...
  const sessionStartedAt =
    isSafeTimestamp(candidate.sessionStartedAt) && candidate.sessionStartedAt <= now ? candidate.sessionStartedAt : null
  const runningSince =
    isSafeTimestamp(candidate.runningSince) && candidate.runningSince <= now ? candidate.runningSince : null
...
const normalizeHistoryEntry = (value: unknown, now: number): IStopwatchHistoryEntry | null => {
  ...
  if (
    ...
    candidate.endedAt > now + MAX_CLOCK_SKEW_MS
  )
    return null
...
export const normalizeStopwatchHistory = (value: unknown, now = Date.now()): IStopwatchHistory => {
  ...
  const rawEntries = Array.isArray((value as Partial<IStopwatchHistory>).entries)
    ? ((value as Partial<IStopwatchHistory>).entries ?? [])
    : []
