...
const lineCoordinate = (value: number | null): number | null =>
  value === null ? null : Math.max(0, Math.min(1, value)) * 100
  ...
  useEffect(() => {
    ...
    if (!getUserMedia) {
      ...
      onSnapshot({
        ...snapshot,
        status: 'unavailable',
        guidance: 'Camera preview is unavailable',
        error: 'This WebView cannot open the camera',
      })
      return undefined
    }
    let disposed = false
    ...
    let landmarker: {
      detectForVideo: (
        ...
        timestampMs: number,
      ) => { landmarks: Array<Array<{ y: number; visibility?: number }>> }
      close: () => void
    } | null = null
    ...
    const start = async (): Promise<void> => {
      try {
        ...
        onSnapshot({
          ...
          status: 'requesting',
          permission: 'authorized',
          guidance: 'Loading local pose tracker…',
          error: null,
        })
          ...
          try {
            if (timestampMs - lastInferenceAt >= 80 && video.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA) {
              ...
              onSnapshot({
                ...
                guidance: completed
                  ? exercise.completionGuidance
                  : trackingLost
                    ? exercise.trackingLostGuidance
                    : update.guidance,
                permission: 'authorized',
            ...
            if (!disposed)
              onSnapshot({
                ...snapshot,
                status: 'error',
                guidance: 'Pose tracking stopped',
                error: error instanceof Error ? error.message : 'Local pose inference failed',
              })
            return
          }
        ...
        onSnapshot({
          ...
          guidance: denied
            ? 'Enable Camera in macOS Settings, or start the break'
            : 'Local shoulder tracking could not start',
    ...
    >
      <header className="movement-header">
        ...
        <span className="movement-local">
          <span className="movement-local-dot" data-active={cameraActive} />
          On this Mac
        </span>
        <button
          ref={closeButtonRef}
      ...
      <div className="movement-body">
        <div
          className="movement-pose-stage"
          aria-label={cameraActive ? `Live ${exercise.title} tracking camera` : 'Movement camera status'}
        >
          {cameraPreviewEnabled ? (
            <video
              ref={videoRef}
              className="movement-camera-preview"
              autoPlay
              muted
              playsInline
              aria-label="Live mirrored Movement Break camera"
            />
          ) : null}
          {!complete && trackingLineY !== null ? (
            <span
              className="movement-tracking-line"
              data-label={exercise.trackingLabel}
              style={{ top: `${trackingLineY}%` }}
              aria-hidden="true"
            />
          ) : null}
          {!complete && targetLineY !== null ? (
            <span
              className="movement-target-line"
              data-label={exercise.targetLabel}
          ...
          {complete ? (
            <div className="movement-success">
              <span className="movement-success-spark is-left" aria-hidden="true">
                ✨
              </span>
              <span className="movement-success-emoji" role="img" aria-label="Celebration">
                🎉
              </span>
              <span className="movement-success-spark is-right" aria-hidden="true">
                ✨
              </span>
              <span>Movement complete</span>
            </div>
          ) : terminalFailure ? (
            <span className="movement-stage-icon">
              <CameraOff size={38} strokeWidth={1.8} />
            </span>
          ...
          {!complete ? (
            <span className="movement-preview-state">
              {previewStatus === 'active'
                ? exercise.previewLabel
        ...
        <div className="movement-progress-copy">
          <div className="movement-count" role="status" aria-live="polite">
            <strong>{snapshot.repCount}</strong>
            <span>/ {snapshot.targetReps}</span>
          </div>
          <div
            className="movement-depth-progress"
            role="progressbar"
            aria-label={exercise.progressLabel}
            aria-valuemin={0}
            aria-valuemax={100}
          ...
          <span className="movement-depth-label">
            {complete ? 'Complete' : `${Math.round(snapshot.progress * 100)}% to target`}
          </span>
      ...
      <footer className="movement-actions">
        ...
        {terminalFailure && allowStartBreak ? (
          <button className="is-primary" type="button" onClick={onStartBreak} disabled={busy}>
            <Play size={13} strokeWidth={2.3} />
            Start break
          </button>
        ) : null}
        {terminalFailure && !allowStartBreak ? (
          <button className="is-primary" type="button" onClick={onCancel} disabled={busy}>
            Back to Pet
          </button>
        ) : null}
