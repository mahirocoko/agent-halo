...
import { getEventActivity, getEventSessionStatus, projectName, shortenPath } from './activity'
import type { ISessionDetail, ISessionSummary, IWorkspaceSessionGroup, SessionEventRegistry } from './types'
...
) => fallbackStatus === 'working' || sessions.some((session) => session.status === 'working')
...
const getSessionWorkspacePath = (events: AgentHaloEvent[], fallback?: string | null): string | null =>
...
export const buildWorkspaceSessionGroups = (sessions: ISessionSummary[]): IWorkspaceSessionGroup[] => {
  const grouped = new Map<string, ISessionSummary[]>()
  for (const session of sessions) {
    const key = session.workspacePath ? `cwd:${session.workspacePath}` : `session:${session.conversationId}`
...
export const buildSessionSummaries = (
  ...
  if (presence.conversationId && !sessions.has(presence.conversationId)) {
    ...
    const current = eventsForSession[0] ? ({ ...eventsForSession[0], cwd: presence.cwd } as AgentHaloEvent) : null
...
export const buildSessionDetail = (
  ...
  const workspacePath = summary.workspacePath ?? getSessionWorkspacePath(sessionEvents, latest?.cwd)
