...
import type { HaloMascotName } from "./HaloMascot";
...
export const SessionContextSummary = ({ mascot, session }: { mascot: HaloMascotName; session: ISessionDetail }) => {
  ...
  return (
    <section className="session-context-summary" data-status={session.status} aria-labelledby="session-context-title" data-panel-focus-target tabIndex={-1}>
      <SessionMascot activityKind={session.activityKind} mascot={mascot} status={session.status} />
...
export interface ISessionListRowProps { child?: boolean; mascot: HaloMascotName; onClear: (id: string) => void; onFocus: (session: ISessionSummary) => void; onOpen: (id: string) => void; session: ISessionSummary; }
export const SessionListRow = ({ child = false, mascot, onClear, onFocus, onOpen, session }: ISessionListRowProps) => (
  <li className={`session-row ${child ? "session-child-row" : ""} ${session.status === "done" ? "ended" : ""}`} data-status={session.status}>
    <button className="session-row-main" type="button" onClick={() => onOpen(session.conversationId)} data-session-id={session.conversationId} data-tauri-drag-region="false" aria-label={`Open ${session.project} session details`}>
      {child ? <StatusGlyph status={session.status} /> : <SessionMascot activityKind={session.activityKind} mascot={mascot} status={session.status} />}
...
export interface IWorkspaceSessionGroupItemProps { expanded: boolean; group: IWorkspaceSessionGroup; groupKey: string; mascot: HaloMascotName; onClear: (id: string) => void; onClearGroup: (group: IWorkspaceSessionGroup) => void; onFocus: (session: ISessionSummary) => void; onOpen: (id: string) => void; onToggle: (key: string) => void; }
export const WorkspaceSessionGroupItem = ({ expanded, group, groupKey, mascot, onClear, onClearGroup, onFocus, onOpen, onToggle }: IWorkspaceSessionGroupItemProps) => {
  if (group.sessions.length === 1) return <SessionListRow mascot={mascot} session={group.sessions[0]} onClear={onClear} onFocus={onFocus} onOpen={onOpen} />;
  return (
    <li className="session-group-block" data-status={group.status}><div className="session-row session-group" data-status={group.status}>
      <button className="session-row-main session-group-main" type="button" onClick={() => onToggle(groupKey)} data-tauri-drag-region="false" aria-expanded={expanded} aria-label={`${expanded ? "Collapse" : "Expand"} ${group.project}, ${group.sessions.length} sessions`}>
        <span className="session-disclosure" aria-hidden="true">{expanded ? <ChevronDown size={12} strokeWidth={2.4} /> : <ChevronRight size={12} strokeWidth={2.4} />}</span><SessionMascot activityKind={group.activityKind} mascot={mascot} status={group.status} />
    ...
    </div>{expanded ? <ul className="session-child-list" aria-label={`${group.project} sessions`}>{group.sessions.map((session) => <SessionListRow child mascot={mascot} session={session} onClear={onClear} onFocus={onFocus} onOpen={onOpen} key={session.conversationId} />)}</ul> : null}</li>
  );
};
