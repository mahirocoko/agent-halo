...
export const normalizeHaloPetMotionMapping = (value: unknown): HaloPetMotionMapping => {
  const candidate =
    value && typeof value === 'object' && 'mapping' in value ? (value as { mapping?: unknown }).mapping : value
  const mapping =
    candidate && typeof candidate === 'object' ? (candidate as Partial<Record<HaloPetSemanticState, unknown>>) : {}
...
const serializeHaloPetMotionMapping = (mapping: HaloPetMotionMapping): string =>
  JSON.stringify({
    schemaVersion: 1,
    mapping,
  })
...
export const writeHaloPetMotionMapping = (mapping: HaloPetMotionMapping): void => {
  try {
    window.localStorage.setItem(
      PET_MOTION_MAPPING_STORAGE_KEY,
      serializeHaloPetMotionMapping(normalizeHaloPetMotionMapping(mapping)),
    )
  } catch {
