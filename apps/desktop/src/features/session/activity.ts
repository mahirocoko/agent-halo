...
export const shortModelName = (model: string): string =>
  (model.split('/').filter(Boolean).at(-1) ?? model).replace(/^chatgpt-plus-pro\//, '') || 'Letta'
...
export const getEventActivity = (event: AgentHaloEvent): IActivityDescriptor => {
  switch (event.type) {
    ...
    case 'llm_end': {
      ...
      const seconds =
        typeof event.data.durationMs === 'number' ? `${Math.max(0.1, event.data.durationMs / 1000).toFixed(1)}s` : null
...
export const staleAfterMsForEvent = (event: AgentHaloEvent): number => {
  ...
  return ['turn_start', 'tool_end', 'compact_end', 'llm_end', 'bridge_error'].includes(event.type)
    ? TRANSITION_STALE_AFTER_MS
    : STALE_AFTER_MS
}

export const getEventSessionStatus = (event: AgentHaloEvent, now = new Date()): ISessionSummary['status'] => {
  ...
  switch (event.type) {
    ...
    case 'tool_end':
      return event.data.status === 'error' ? (inactive ? 'inactive' : 'error') : inactive ? 'inactive' : 'working'
    case 'llm_end':
