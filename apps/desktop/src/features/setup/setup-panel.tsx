...
import {
  getHaloBotLoadoutLabel,
  getHaloBotParts,
  HALO_BOT_COMBINATION_COUNT,
  HALO_BOT_PART_CATALOG,
  HALO_BOT_PART_CATEGORIES,
  type HaloBotLoadout,
  type HaloBotPartCategory,
  setHaloBotPart,
} from '../session/halo-bot'
import { HALO_PET_ROSTER, HaloBotBody, type HaloPetName } from '../session/halo-pet'
import {
...
const completionPetSizeLabel = (size: CompletionPetSize): string =>
...
const SetupPetPreview = ({
  ...
  <span
    className="pet-option-sprite"
    data-pet={pet}
    data-compact={compact}
    style={petPreviewStyle(pet, loadout, compact)}
    aria-hidden="true"
  >
...
export const SetupPanel = ({
  ...
  const motionMappingIsDefault = HALO_PET_MOTIONS.every(
    (state) => petMotionMapping[state] === DEFAULT_HALO_PET_MOTION_MAPPING[state],
  )
  ...
  const handleDisplayKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>, currentIndex: number): void => {
    ...
    if (
      ...
      displays.length === 0 ||
      displayInteractionBusyRef.current ||
      displayLoading
    )
      return
    event.preventDefault()
    ...
    const nextIndex =
      event.key === 'Home'
        ...
        : event.key === 'End'
          ? displays.length - 1
          : (currentIndex + (['ArrowRight', 'ArrowDown'].includes(event.key) ? 1 : -1) + displays.length) %
            displays.length
  ...
  const handleCategoryKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>, current: SetupCategory): void => {
    ...
    const nextIndex =
      event.key === 'Home'
        ? 0
        : event.key === 'End'
          ? SETUP_CATEGORIES.length - 1
          : (currentIndex + (['ArrowRight', 'ArrowDown'].includes(event.key) ? 1 : -1) + SETUP_CATEGORIES.length) %
            SETUP_CATEGORIES.length
  ...
  const handlePetSizeKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>, current: CompletionPetSize): void => {
    ...
    const nextIndex =
      event.key === 'Home'
        ? 0
        : event.key === 'End'
          ? COMPLETION_PET_SIZES.length - 1
          : (currentIndex + (['ArrowRight', 'ArrowDown'].includes(event.key) ? 1 : -1) + COMPLETION_PET_SIZES.length) %
            COMPLETION_PET_SIZES.length
  ...
  useEffect(() => {
    ...
    const selectedIndex = Math.max(
      0,
      displays.findIndex((display) => display.id === displayFocusTarget),
    )
  ...
  return (
    <div className="setup-tray" data-testid="settings-board" onKeyDown={handleSetupKeyDown} ref={layout.trayRef}>
      <BoardSurface
        className="setup-card setup-navigation-card"
        tone="slate"
        aria-label="Setup navigation"
        style={layout.cardStyle(0)}
      >
        <BoardScroll data-setup-card="navigation">
          <div className="setup-navigation-heading">
            <strong>Setup</strong>
            <small>Agent Halo preferences</small>
          </div>
          <div
          ...
          >
            <button
              className="setup-side-tab"
              id="setup-tab-connection"
              type="button"
              role="tab"
              aria-selected={activeCategory === 'connection'}
              aria-controls="setup-panel-connection"
              tabIndex={activeCategory === 'connection' ? 0 : -1}
              data-active={activeCategory === 'connection'}
              onClick={() => selectCategory('connection')}
              onKeyDown={(event) => handleCategoryKeyDown(event, 'connection')}
            >
              <PlugZap size={12} strokeWidth={2.2} />
            ...
            <button
              ...
              type="button"
              role="tab"
              aria-selected={activeCategory === 'pet'}
              aria-controls="setup-panel-pet"
              tabIndex={activeCategory === 'pet' ? 0 : -1}
              data-active={activeCategory === 'pet'}
              onClick={() => selectCategory('pet')}
              onKeyDown={(event) => handleCategoryKeyDown(event, 'pet')}
            >
            ...
            <button
              className="setup-side-tab"
              id="setup-tab-display"
              type="button"
              role="tab"
              aria-selected={activeCategory === 'display'}
              aria-controls="setup-panel-display"
              tabIndex={activeCategory === 'display' ? 0 : -1}
              data-active={activeCategory === 'display'}
              onClick={() => selectCategory('display')}
      ...
      <BoardSurface
        className="setup-card setup-detail-card"
        tone="parchment"
        aria-label={`${activeCategoryCopy.title} settings`}
        style={layout.cardStyle(1)}
      >
        <div className="setup-detail-heading">
          <span>{activeCategoryCopy.title}</span>
          <small>{activeCategoryCopy.detail}</small>
        </div>
        <BoardScroll className="setup-detail-body" data-setup-card="detail">
          <div
            className="setup-category-panel"
            id={`setup-panel-${activeCategory}`}
            role="tabpanel"
            aria-labelledby={`setup-tab-${activeCategory}`}
          >
            {activeCategory === 'connection' ? (
              <>
                <div className="setup-row">
                  ...
                  <button className="pill-btn" type="button" onClick={onCheckBridge} data-tauri-drag-region="false">
                ...
                <div className="setup-row">
                  ...
                  <span className="setup-copy">
                    <span className="setup-title">AGY hooks</span>
                    <span className="setup-detail">
                      {agyHookStatus.installed === true
                        ? `Installed · ${shortenPath(agyHookStatus.path)}`
                        : agyHookStatus.installed === false
                          ? `Not installed · ${shortenPath(agyHookStatus.path)}`
                          : canUseNativeControls
                            ? 'Checking install state'
                            : 'Tauri runtime needed'}
                    </span>
                ...
                <div className="setup-row passive">
                  <span className="status-slot">
                    <ArrowRight className="setup-icon" size={14} strokeWidth={2.3} />
                  </span>
                  <span className="setup-copy">
                    <span className="setup-title">{guidance.title}</span>
                    <span className="setup-detail">{guidance.detail}</span>
                  </span>
                </div>
                <div className="setup-row passive">
                  <span className="status-slot">
                    <Focus className="setup-icon" size={14} strokeWidth={2.3} />
                  </span>
                  <span className="setup-copy">
                    <span className="setup-title">Session controls</span>
                    <span className="setup-detail">
                      {canUseNativeControls
                        ? 'Herdr exact focus · Ghostty fallback · end unavailable'
                ...
                {nativeAction.message ? (
                  <div
                    ...
                    data-online={nativeAction.bridgeOnline === true}
                    role="status"
                    aria-live="polite"
                  >
                    {nativeAction.message}
            ...
            {activeCategory === 'pet' ? (
              <>
                <div className="setup-row pet-setting-row">
                  <span className="pet-current-preview">
                    <SetupPetPreview pet={pet} loadout={haloBotLoadout} />
                  </span>
                  <span className="setup-copy">
                    <span className="setup-title">{PET_LABELS[pet]}</span>
                    <span className="setup-detail" id="pet-preview-availability">
                      {pet === 'halo-bot'
                        ? `${getHaloBotLoadoutLabel(haloBotLoadout)} · ${haloBotLoadout.toUpperCase()} · used across Agent Halo`
                        : canUseNativeControls
                          ? 'Used across Agent Halo'
                          : 'Desktop runtime required to preview'}
                    </span>
                  </span>
                  <span className="setup-row-actions">
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        if (petPickerOpen) closePetPicker()
                        else {
                          setLoadoutPickerOpen(false)
                      ...
                      data-tauri-drag-region="false"
                      aria-controls="pet-picker"
                      aria-expanded={petPickerOpen}
                    >
                      <Bot size={12} strokeWidth={2.3} />
                      {petPickerOpen ? 'Close' : 'Choose'}
                    </button>
                    <button
                      className={`pill-btn accent pet-preview-button ${petPreviewState === 'stale' ? 'is-stale' : ''}`}
                      type="button"
                      disabled={!canUseNativeControls || petPreviewState === 'showing'}
                      onClick={() => void onShowPetPreview()}
                      data-tauri-drag-region="false"
                      aria-label={
                        petPreviewState === 'stale' ? 'Update Completion Pet preview' : 'Show Completion Pet preview'
                      }
                      aria-describedby="pet-preview-availability"
                    >
                      {petPreviewState === 'stale' ? (
                        <RefreshCw size={12} strokeWidth={2.3} />
                      ) : petPreviewState === 'shown' ? (
                        <Check size={12} strokeWidth={2.3} />
                      ) : (
                        <Play size={12} strokeWidth={2.3} />
                      )}
                      {petPreviewState === 'showing'
                        ? 'Showing…'
                        : petPreviewState === 'stale'
                          ? 'Update Pet'
                          : petPreviewState === 'shown'
                  ...
                  <div className="pet-picker" id="pet-picker" role="radiogroup" aria-label="Pet">
                    {HALO_PET_ROSTER.map((option) => (
                      <button
                        ...
                        data-selected={option === pet}
                        id={`pet-option-${option}`}
                        type="button"
                        role="radio"
                        aria-checked={option === pet}
                        tabIndex={option === pet ? 0 : -1}
                        onClick={() => {
                          onPetChange(option)
                          closePetPicker()
                        }}
                        onKeyDown={(event) => handlePetKeyDown(event, option)}
                        data-tauri-drag-region="false"
                        key={option}
                        title={PET_LABELS[option]}
                      >
                        <SetupPetPreview compact pet={option} loadout={haloBotLoadout} />
                        <span>{PET_LABELS[option]}</span>
                      </button>
                    ))}
                  </div>
                ) : null}
                {pet === 'halo-bot' && !petPickerOpen ? (
                  <div className="halo-bot-loadout-disclosure">
                    <span>
                      Pixabot · {haloBotLoadout.toUpperCase()} · {HALO_BOT_COMBINATION_COUNT.toLocaleString()}{' '}
                      combinations
                    </span>
                    <button
                      ref={loadoutPickerTriggerRef}
                      className="pill-btn"
                      type="button"
                      aria-controls="halo-bot-loadout-picker"
                      aria-expanded={loadoutPickerOpen}
                      onClick={() => setLoadoutPickerOpen((current) => !current)}
                      data-tauri-drag-region="false"
                    >
                      {loadoutPickerOpen ? 'Close' : 'Change'}
                    </button>
                  </div>
                ) : null}
                {pet === 'halo-bot' && loadoutPickerOpen && !petPickerOpen ? (
                  <div
                    className="halo-bot-loadout-picker"
                    id="halo-bot-loadout-picker"
                    role="group"
                    aria-label="Halo Bot loadout"
                  >
                    <div className="halo-bot-loadout-preview">
                      <SetupPetPreview pet="halo-bot" loadout={haloBotLoadout} />
                      <span>
                        <strong>{getHaloBotLoadoutLabel(haloBotLoadout)}</strong>
                        <small>ID {haloBotLoadout.toUpperCase()}</small>
                      </span>
                    </div>
                      ...
                      {HALO_BOT_PART_CATEGORIES.map((category) => {
                        ...
                        return (
                          <label className="halo-bot-part-row" htmlFor={`halo-bot-part-${category}`} key={category}>
                            ...
                            <select
                              id={`halo-bot-part-${category}`}
                              value={selected.index}
                              onChange={(event) => handleHaloBotPartChange(category, event.target.value)}
                              data-tauri-drag-region="false"
                              aria-label={`Halo Bot ${HALO_BOT_PART_LABELS[category]}`}
                            >
                              {HALO_BOT_PART_CATALOG[category].map((part, index) => (
                                <option value={index} key={part.name}>
                                  {part.name}
                                </option>
                              ))}
                            </select>
                ...
                <div className="setup-row">
                  ...
                  <span className="setup-copy">
                    <span className="setup-title">Completion Pet size</span>
                    <span className="setup-detail">Changes only the floating Pet</span>
                  </span>
                  <span className="setup-size-options" role="radiogroup" aria-label="Completion Pet size">
                    {COMPLETION_PET_SIZES.map((size) => (
                      <button
                        id={`completion-pet-size-${size}`}
                        type="button"
                        role="radio"
                        aria-checked={completionPetSize === size}
                        tabIndex={completionPetSize === size ? 0 : -1}
                        data-active={completionPetSize === size}
                        onClick={() => onCompletionPetSizeChange(size)}
                        onKeyDown={(event) => handlePetSizeKeyDown(event, size)}
                        data-tauri-drag-region="false"
                        key={size}
                      >
                        {completionPetSizeLabel(size)}
                      </button>
                    ))}
                  </span>
                </div>
                <div className="setup-row">
                  ...
                  <button
                    className={`pill-btn ${completionPetEnabled ? 'accent' : ''}`}
                    type="button"
                    role="switch"
                ...
                <div className="setup-row">
                  <span className="status-slot">
                    <Dumbbell className="setup-icon" size={14} strokeWidth={2.3} />
                  </span>
                  <span className="setup-copy">
                    <span className="setup-title">Offer movement after Focus</span>
                    <span className="setup-detail">
                      {movementBreakEnabled
                        ? 'Squats or reaches appear with Focus completion'
                        : 'Off after Focus · manual Move remains available'}
                    </span>
                  </span>
                  <button
                    className={`pill-btn ${movementBreakEnabled ? 'accent' : ''}`}
                    type="button"
                    role="switch"
                    aria-checked={movementBreakEnabled}
                    onClick={() => onMovementBreakEnabledChange(!movementBreakEnabled)}
                    data-tauri-drag-region="false"
                    aria-label={`${movementBreakEnabled ? 'Disable' : 'Enable'} movement offer after Focus`}
                  >
                    {movementBreakEnabled ? 'On' : 'Off'}
                  </button>
                </div>
                {movementBreakEnabled ? (
                  <div className="notice-row movement-privacy-note" role="note">
                    Camera opens only after a specific exercise is clicked. Pose analysis stays on this Mac; no video or
                    audio is saved.
                  </div>
                ) : null}
                {!petPickerOpen && !loadoutPickerOpen ? (
                  <div className="pet-motion-mapping" role="group" aria-labelledby="pet-motion-mapping-title">
                    <div className="pet-motion-mapping-head">
                      <span>
                        <strong id="pet-motion-mapping-title">Letta motion mapping</strong>
                        <small>Choose body motion per state · status and Signal do not change</small>
                      </span>
                      <button
                        className="pill-btn"
                        type="button"
                        disabled={motionMappingIsDefault}
                        onClick={onPetMotionReset}
                        data-tauri-drag-region="false"
                      >
                        Reset
                      </button>
                    ...
                    <div className="pet-motion-mapping-grid">
                      {HALO_PET_MOTIONS.map((state) => (
                        <label className="pet-motion-row" htmlFor={`pet-motion-${state}`} key={state}>
                          ...
                          <select
                            id={`pet-motion-${state}`}
                            value={petMotionMapping[state]}
                            onChange={(event) => onPetMotionChange(state, event.target.value as HaloPetMotion)}
                            data-tauri-drag-region="false"
                            aria-label={`${PET_MOTION_LABELS[state]} Letta state motion`}
                            title={`${PET_MOTION_LABELS[state]} uses ${PET_MOTION_LABELS[petMotionMapping[state]]} motion`}
                          >
                            {HALO_PET_MOTIONS.map((motion) => (
                              <option value={motion} key={motion}>
                                {PET_MOTION_OPTION_LABELS[motion]}
                              </option>
                            ))}
                          </select>
                        </label>
                ...
                {petPreviewStatus ? (
                  <div
                    className="notice-row pet-preview-status"
                    data-state={petPreviewState}
                    data-online={petPreviewState === 'shown'}
                    role="status"
                    aria-live="polite"
                  >
                    {petPreviewStatus}
            ...
            {activeCategory === 'display' ? (
              <>
                <div className="setup-row display-setting-row">
                  <span className="status-slot">
                    <MonitorIcon className="setup-icon" size={14} strokeWidth={2.2} />
                  </span>
                  <span className="setup-copy">
                    <span className="setup-title">Target display</span>
                    <span className="setup-detail">
                      {!canUseNativeControls
                        ? 'Desktop runtime required'
                        : displayLoading
                          ? 'Reading connected displays'
                          : displayError
                            ? displayError
                            : displayState?.fallbackActive
                              ? `${displayState.preferredDisplayName || 'Saved display'} unavailable · using ${activeDisplay?.name ?? 'Primary'}`
                              : activeDisplay
                                ? `${activeDisplay.name} · ${displayResolutionLabel(activeDisplay)}${activeDisplay.isPrimary ? ' · Primary' : ''}`
                                : 'No connected display found'}
                    </span>
                  </span>
                  <button
                    ref={displayPickerTriggerRef}
                    className="pill-btn"
                    type="button"
                    disabled={!canUseNativeControls || displays.length === 0}
                    aria-busy={displayLoading}
                    onClick={() => {
                      if (displayPickerOpen) closeDisplayPicker()
                      else {
                    ...
                    data-tauri-drag-region="false"
                    aria-controls="display-picker"
                    aria-expanded={displayPickerOpen}
                  >
                    <MonitorIcon size={12} strokeWidth={2.2} />
                    {displayPickerOpen ? 'Close' : 'Choose'}
                  </button>
                </div>
                {displayPickerOpen ? (
                  <div
                  ...
                  >
                    {displays.map((display, index) => (
                      <button
                        ...
                        data-selected={display.id === displayRadioSelection}
                        disabled={displayLoading}
                        id={`display-option-${index}`}
                        type="button"
                        role="radio"
                        aria-checked={display.id === displayRadioSelection}
                        tabIndex={display.id === displayFocusTarget ? 0 : -1}
                        onClick={() => {
                          if (displayInteractionBusyRef.current) return
                          displayInteractionBusyRef.current = true
                          void onDisplayChange(display.id).finally(() => {
                            displayInteractionBusyRef.current = false
                          })
                          closeDisplayPicker()
                        }}
                      ...
                      >
                        ...
                        <span className="display-option-copy">
                          ...
                          <small>
                            {displayResolutionLabel(display)}
                            {display.isPrimary ? ' · Primary' : ''}
                          </small>
                        </span>
                        <span className="display-option-mark" aria-hidden="true">
                          {display.id === displayRadioSelection ? '✓' : ''}
                        </span>
                      </button>
                    ))}
                  </div>
                ...
                <div className="setup-row">
                  <span className="status-slot">
                    <Coffee className="setup-icon" size={14} strokeWidth={2.3} />
                  </span>
                  <span className="setup-copy">
