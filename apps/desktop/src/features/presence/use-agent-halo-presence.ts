...
import { appendRecentEvent, mergeSessionEvents, normalizeSessionEventIdentity } from '../session/event-registry'
import { readSessionEventRegistry, writeSessionEventRegistry } from '../session/persistence'
...
const readBoolean = (value: unknown, key: string, fallback: boolean) =>
  typeof value === 'object' && value !== null && typeof (value as Record<string, unknown>)[key] === 'boolean'
...
const normalizeCapabilities = (value: unknown): IAgentHaloBridgeCapabilities => {
  ...
  return {
    ...
    endpoints: {
      ...
      hookAttention: readBoolean(record.endpoints, 'hookAttention', fallback.endpoints.hookAttention),
      ingest: readBoolean(record.endpoints, 'ingest', fallback.endpoints.ingest),
    },
    sessionActions: {
      focusTerminal: readBoolean(record.sessionActions, 'focusTerminal', fallback.sessionActions.focusTerminal),
      endSession: readBoolean(record.sessionActions, 'endSession', fallback.sessionActions.endSession),
      dismissEnded: readBoolean(record.sessionActions, 'dismissEnded', fallback.sessionActions.dismissEnded),
    },
  }
}
...
const createScenario = (scenario: string): AgentHaloEvent[] => {
  ...
  if (scenario === 'multi') {
    ...
    const item = (conversationId: string, cwd: string) => ({
      ...common,
      conversationId,
      cwd,
      runtime: demoRuntime(conversationId),
    })
    return [
      {
