...
export const runtimeTargetKey = (
  target: Pick<IRuntimeUsageTarget, 'conversationId' | 'processId' | 'sourceStartedAtMs'>,
): string => `${target.processId}:${target.sourceStartedAtMs}:${target.conversationId}`

export const isTerminalRuntimeStatus = (status: IRuntimeUsageSnapshot['status']): boolean =>
  status === 'missing' || status === 'pidReused'
...
const buildRuntimeTarget = (
  ...
  return {
    ...
    herdrPaneId:
      session.herdrTarget?.sourcePid === runtimeEvent.runtime.sourcePid &&
...
export const buildLocalServiceOwnerTargets = ({
  sessions,
  registry,
}: IRuntimeTargetSource): ILocalServiceOwnerTarget[] => {
...
export const selectRuntimeSamplingTargets = (
  targets: IRuntimeUsageTarget[],
  endedIdentities: ReadonlyMap<string, number>,
): IRuntimeUsageTarget[] =>
...
export const selectRuntimeMonitorTargets = (
  ...
  const candidates = processActive ? [...livenessTargets, ...runtimeTargets] : livenessActive ? livenessTargets : []
...
export const classifyRuntimePressure = (
  ...
  if (children >= 3 * GIB || childCpu >= 250)
    return { pressure: 'critical', pressureReason: 'Child workload is using several cores or over 3 GiB' }
  if (host >= 1.5 * GIB)
    return { pressure: 'high', pressureReason: quiet ? 'High memory while quiet' : 'Host above 1.5 GiB' }
...
export const buildRuntimeSessionViews = (
  ...
  return targets
    .map((target) => {
      ...
      const snapshot =
        candidate?.targetSourceStartedAtMs === target.sourceStartedAtMs &&
        (candidate.processStartTimeMs == null ||
          Math.abs(candidate.processStartTimeMs - target.sourceStartedAtMs) <= 2_000)
          ? candidate
          : null
...
export const formatLocalServiceEndpoint = (service: Pick<ILocalService, 'bindAddress' | 'port'>): string => {
  const host =
    service.bindAddress.includes(':') && !service.bindAddress.startsWith('[')
      ? `[${service.bindAddress}]`
      : service.bindAddress
