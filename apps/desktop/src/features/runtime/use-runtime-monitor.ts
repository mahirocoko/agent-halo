...
import {
  ...
  buildRuntimeUsageTargets,
  createDemoLocalServices,
...
import {
  mergeRuntimeEndedIdentities,
  readRuntimeEndedIdentities,
  reconcileRuntimeEndedIdentities,
  writeRuntimeEndedIdentities,
} from './persistence'
import type {
  ILocalService,
  ...
  IRuntimeMonitorView,
  IRuntimeNativeTarget,
  IRuntimeTargetSource,
  IRuntimeUsageSnapshot,
  IRuntimeUsageTarget,
} from './types'
...
export const useRuntimeMonitor = ({
  ...
  servicesActive,
  sessions,
}: IUseRuntimeMonitorOptions): IRuntimeMonitorView => {
  ...
  const eligibleTargets = useMemo(
    () => allTargets.filter((target) => !endedIdentities.has(runtimeTargetKey(target))),
    [allTargets, endedIdentities],
  )
  const runtimeTargets = useMemo(
    () => selectRuntimeSamplingTargets(allTargets, endedIdentities),
    [allTargets, endedIdentities],
  )
  ...
  const referencedTargets = useMemo(() => {
    const byIdentity = new Map(
      [...allTargets, ...livenessCandidates].map((target) => [runtimeTargetKey(target), target]),
    )
  ...
  const targetKey = useMemo(
    () =>
      samplingTargets
        .map(
          (target) =>
            `${target.processId}:${target.sourceStartedAtMs}:${target.conversationId}:${target.runtimeEventId}:${target.cwd ?? ''}`,
        )
        .join('|'),
    [samplingTargets],
  )
  ...
  const acceptSnapshots = useCallback((next: IRuntimeUsageSnapshot[], current: IRuntimeUsageTarget[]) => {
    const targetsByNativeIdentity = new Map(
      current.map((target) => [`${target.processId}:${target.conversationId}`, target]),
    )
    const attributed = next.map((snapshot) => ({
      ...snapshot,
      targetSourceStartedAtMs:
        targetsByNativeIdentity.get(`${snapshot.processId}:${snapshot.conversationId}`)?.sourceStartedAtMs ?? null,
    }))
    ...
    const endedTargets = current.filter((target) =>
      terminalIdentities.has(`${target.processId}:${target.conversationId}`),
    )
    if (endedTargets.length > 0)
      setEndedIdentities((previous) => mergeRuntimeEndedIdentities(previous, endedTargets, allTargetsRef.current))
  ...
  const refreshServices = useCallback(async () => {
    ...
    if (demoMode) {
      if (servicesRequestVersionRef.current === requestVersion) {
        setServices(
          createDemoLocalServices().filter(
            (service) => !demoStoppedProcessKeysRef.current.has(localServiceProcessKey(service)),
          ),
        )
  ...
  const controlLocalService = useCallback(
    async (request: ILocalServiceControlRequest): Promise<ILocalServiceControlResult> => {
      if (demoMode) {
  ...
  const refreshProcesses = useCallback(async () => {
    ...
    if (demoMode) {
      ...
      if (processRequestVersionRef.current === requestVersion && targetKeyRef.current === requestTargetKey)
        acceptSnapshots(next, current)
      return
    }
    if (!canUseNativeControls) {
      setSnapshots([])
    ...
    setLoading(true)
    try {
      const nativeTargets: IRuntimeNativeTarget[] = current.map(
        ({ conversationId, runtimeEventId, processId, sourceStartedAtMs, cwd }) => ({
          conversationId,
