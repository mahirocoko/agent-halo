...
export const readUsageSettings = (): IUsageSettings => {
  try {
    const parsed = JSON.parse(window.localStorage.getItem(STORAGE_KEY) ?? 'null') as Partial<IUsageSettings> | null
    ...
    return {
      refreshMs:
        typeof parsed.refreshMs === 'number' && VALID_REFRESH_MINUTES.includes(parsed.refreshMs / 60_000)
          ? parsed.refreshMs
...
export const formatAbsoluteTime = (timestamp: string, format: UsageTimeFormat): string | null => {
...
export const formatResetLabel = (resetsAt: string | undefined, settings: IUsageSettings): string | null => {
  if (!resetsAt) {
    return null
  }
