...
const createInitialUsageStates = (settings: IUsageSettings): Record<UsageProviderId, IAgentUsageState> => {
  ...
  return Object.fromEntries(
    USAGE_PROVIDERS.map((provider) => {
      ...
      return [
        provider.id,
        snapshot ? createCachedAgentUsageState(provider.id, snapshot, settings) : createAgentUsageState(provider.id),
      ]
    }),
  ) as Record<UsageProviderId, IAgentUsageState>
}

export const useAgentUsageList = (settings: IUsageSettings, demoMode: boolean): IAgentUsageListResult => {
  ...
  const fetchProvider = async (provider: IUsageProviderConfig): Promise<void> => {
    if (demoMode) {
      setUsages((current) => ({
        ...current,
    ...
    try {
      ...
      const next = parseAgentUsageSnapshot(provider.id, snapshot, settingsRef.current)
      ...
      setUsages((current) => ({
        ...current,
        [provider.id]:
          next.status === 'error' && next.message
      ...
      setUsages((current) => ({
        ...
        [provider.id]: retainLastGoodUsage(
          current[provider.id],
          provider.id,
          error instanceof Error ? error.message : String(error || `${provider.label} usage unavailable`),
        ),
      }))
    }
  }
  ...
  }, [tick, settings.resetMode, settings.timeFormat, settings.usageMode, snapshots])
